...
export class BZip2Decompressor {
    public static state: Bzip2Context; public static state_$LI$(): Bzip2Context { if (BZip2Decompressor.state == null) { BZip2Decompressor.state = new Bzip2Context(); } return BZip2Decompressor.state; }

    public static decompress$byte_A$int$byte_A$int$int(output: number[], lenght: number, compressed: number[], decompressedLength: number, minLen: number): number {
        {
    ...
    public static decompress(output?: any, lenght?: any, compressed?: any, decompressedLength?: any, minLen?: any): any {
        if (((output != null && output instanceof Array as any && (output.length == 0 || output[0] == null || (typeof output[0] === "number"))) || output === null) && ((typeof lenght === "number") || lenght === null) && ((compressed != null && compressed instanceof Array as any && (compressed.length == 0 || compressed[0] == null || (typeof compressed[0] === "number"))) || compressed === null) && ((typeof decompressedLength === "number") || decompressedLength === null) && ((typeof minLen === "number") || minLen === null)) {
            return BZip2Decompressor.decompress$byte_A$int$byte_A$int$int(output, lenght, compressed, decompressedLength, minLen) as any;
        } else if (((output != null && output instanceof Bzip2Context as any) || output === null) && lenght === undefined && compressed === undefined && decompressedLength === undefined && minLen === undefined) {
    ...
    public static method313(bzip2Context: Bzip2Context) {
        ...
        if (bzip2Context.totalOutLo32 < i2) { bzip2Context.totalOutHigh32++; }
    ...
    public static decompress$com_jagex_runescape_cache_bzip_Bzip2Context(bzip2Context: Bzip2Context) {
        ...
        if (Bzip2Context.anIntArray71 == null) { Bzip2Context.anIntArray71 = ((s) => { const a = []; while (s-- > 0) { a.push(0); } return a; })(bzip2Context.anInt62 * 100000); }
        ...
        while ((flag19)) {{
            ...
            if (b !== 0) { bzip2Context.aBoolean59 = true; } else { bzip2Context.aBoolean59 = false; }
            if (bzip2Context.aBoolean59) { console.info("PANIC! RANDOMISED BLOCK!"); }
            ...
            for (let j: number = 0; j < 16; j++) {{
                ...
                if (byte1 === 1) { bzip2Context.aBooleanArray74[j] = true; } else { bzip2Context.aBooleanArray74[j] = false; }
            }}
            for (let k: number = 0; k < 256; k++) {bzip2Context.aBooleanArray73[k] = false; }
            for (let l: number = 0; l < 16; l++) {if (bzip2Context.aBooleanArray74[l]) {
                for (let i3: number = 0; i3 < 16; i3++) {{
                    const byte2: number = BZip2Decompressor.method316(bzip2Context);
                    if (byte2 === 1) { bzip2Context.aBooleanArray73[l * 16 + i3] = true; }
                }}
            }}
            ...
            for (let byte16: number = 0; byte16 < j4; byte16++) {abyte0[byte16] = byte16; }
            ...
            for (let l3: number = 0; l3 < j4; l3++) {{
                ...
                for (let l1: number = 0; l1 < i4; l1++) {{
                    if (bzip2Context.aByteArrayArray80[l3][l1] > i) { i = bzip2Context.aByteArrayArray80[l3][l1]; }
                    if (bzip2Context.aByteArrayArray80[l3][l1] < byte8) { byte8 = bzip2Context.aByteArrayArray80[l3][l1]; }
                }}
                BZip2Decompressor.method319(bzip2Context.anIntArrayArray81[l3], bzip2Context.anIntArrayArray82[l3], bzip2Context.anIntArrayArray83[l3], bzip2Context.aByteArrayArray80[l3], byte8, i, i4);
                bzip2Context.anIntArray84[l3] = byte8;
            }}
            const l4: number = bzip2Context.anInt72 + 1;
            let i5: number = -1;
            let j5: number = 0;
            for (let i2: number = 0; i2 <= 255; i2++) {bzip2Context.anIntArray67[i2] = 0; }
            let j9: number = 4095;
            ...
            for (let k5: number = ai2[l7 - ai1[i7]]; k5 !== l4; ) {if (k5 === 0 || k5 === 1) {
                let j6: number = -1;
                let k6: number = 1;
                do {{
                ...
                if (j11 < 16) {
                    ...
                    for (; j11 > 0; j11--) {bzip2Context.aByteArray76[j10 + j11] = bzip2Context.aByteArray76[(j10 + j11) - 1]; }
                    ...
                    for (; k10 > bzip2Context.anIntArray77[l10]; k10--) {bzip2Context.aByteArray76[k10] = bzip2Context.aByteArray76[k10 - 1]; }
            ...
            for (let j2: number = 1; j2 <= 256; j2++) {bzip2Context.anIntArray69[j2] = bzip2Context.anIntArray67[j2 - 1]; }
            for (let k2: number = 1; k2 <= 256; k2++) {bzip2Context.anIntArray69[k2] += bzip2Context.anIntArray69[k2 - 1]; }
            ...
            if (bzip2Context.anInt68 === bzip2Context.anInt85 + 1 && bzip2Context.anInt58 === 0) { flag19 = true; } else { flag19 = false; }
        }}
    }
    ...
    public static method317(i: number, class1: Bzip2Context): number {
        ...
        do {{
            ...
            class1.totalInLo32++;
            if (class1.totalInLo32 === 0) { class1.totalInHi32++; }
        }} while ((true));
        return j;
    }
    ...
    public static method318(class1: Bzip2Context) {
        class1.anInt72 = 0;
        for (let i: number = 0; i < 256; i++) {if (class1.aBooleanArray73[i]) {
    ...
    public static method319(ai: number[], ai1: number[], ai2: number[], abyte0: number[], i: number, j: number, k: number) {
        ...
        for (let i1: number = i; i1 <= j; i1++) {{
            for (let l2: number = 0; l2 < k; l2++) {if (abyte0[l2] === i1) {
                ai2[l] = l2;
                l++;
            }}
        }}
        for (let j1: number = 0; j1 < 23; j1++) {ai1[j1] = 0; }
        for (let k1: number = 0; k1 < k; k1++) {ai1[abyte0[k1] + 1]++; }
        for (let l1: number = 1; l1 < 23; l1++) {ai1[l1] += ai1[l1 - 1]; }
        for (let i2: number = 0; i2 < 23; i2++) {ai[i2] = 0; }
        let i3: number = 0;
        ...
        for (let k2: number = i + 1; k2 <= j; k2++) {ai1[k2] = (ai[k2 - 1] + 1 << 1) - ai1[k2]; }
    }
}
